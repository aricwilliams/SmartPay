...
export const Wallet: React.FC = () => {
  ...
  const filteredTransactions = selectedWallet 
    ? transactions.filter(tx => tx.walletId === selectedWallet)
    : transactions;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {wallets.map((wallet, index) => (
          ...
          >
            <Card 
              hoverable 
              className={`cursor-pointer ${selectedWallet === wallet.id ? 'ring-2 ring-blue-500' : ''}`}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`p-2 rounded-full ${
                      wallet.type === 'crypto' ? 'bg-purple-100' : 'bg-blue-100'
                    }`}>
                      {wallet.type === 'crypto' ? (
                        <BanknotesIcon className={`w-5 h-5 ${
                          wallet.type === 'crypto' ? 'text-purple-600' : 'text-blue-600'
                        }`} />
                      ) : (
