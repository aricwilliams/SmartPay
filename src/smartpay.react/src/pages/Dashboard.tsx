...
import { CurrencyDollarIcon, BriefcaseIcon, ArrowTrendingUpIcon, ClockIcon } from "@heroicons/react/24/outline";
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="volume" stroke="url(#colorGradient)" strokeWidth={3} dot={{ fill: "#3B82F6", strokeWidth: 2 }} />
                <defs>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData}>
                ...
                <YAxis />
                <Tooltip />
                <Line type="monotone" dataKey="jobs" stroke="url(#colorGradient2)" strokeWidth={3} dot={{ fill: "#10B981", strokeWidth: 2 }} />
                <defs>
            ...
            {recentTransactions.map((transaction) => (
              <div key={transaction.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-4">
                  <div className={`p-2 rounded-full ${transaction.type === "release" ? "bg-green-100" : transaction.type === "escrow" ? "bg-orange-100" : "bg-blue-100"}`}>
                    <CurrencyDollarIcon className={`w-5 h-5 ${transaction.type === "release" ? "text-green-600" : transaction.type === "escrow" ? "text-orange-600" : "text-blue-600"}`} />
                  </div>
                  <div>
                    <p className="font-medium text-gray-900">{transaction.job}</p>
                    <p className="text-sm text-gray-500">
                      {transaction.type === "release" ? "Milestone Payment Released" : transaction.type === "escrow" ? "Escrow Created" : transaction.type === "deposit" ? "Funds Received" : "Transaction"}
                      {transaction.jobId && <span className="ml-2 text-blue-600 font-medium">• Job Payment</span>}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-500">{formatRelativeTime(transaction.timestamp)}</span>
                  </div>
                </div>
              </div>
            ))}
